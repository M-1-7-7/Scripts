...
Foreach ($POWERPOINTstring in $POWERPOINTfile)
{
    if ($POWERPOINTstring -like "*``]*" -or $POWERPOINTstring -like "*``[*" ){    
        Add-Content C:\Users\User\Desktop\output\NotAcceptedPowerpoint.txt $POWERPOINTstring.Path
    }
    else{
...
Foreach ($EXCELstring in $EXCELfile)
{
    if ($EXCELstring -like "*``]*" -or $EXCELstring -like "*``[*" ){
        Add-Content C:\Users\User\Desktop\output\NotAcceptedExcel.txt $EXCELstring.Path
    }
    else{
