...
def check_user_input():
    ...
    if not os.path.exists(start_dir_Var.get()): 
        print("Please enter a valid STARTING DIRECTORY")
        start_dir_status = "Please enter a valid STARTING DIRECTORY"
    else:
        print("starting directory is existing")
        start_dir_status = "starting directory is existing"
    ...
    try:
        with open(DW_Var.get(), "r"):
            print("we can open dirty word txt")
    except PermissionError:
        print("Not Accesable")

    text_widget = Text(root, height = 5, 
              width = 25, 
              bg = "light cyan")
    text_widget.grid(column=5, row=5)
    text_widget.insert(END, start_dir_status)
...
def search_files():
    ...
    #if isFileWritable:
        #print(f"{dirty_word_file} is writable.")
    #else:
        #print(f"{dirty_word_file} is not writable.")
...
def main_screen():
    ...
    All_Excel_Button.grid(row=2, columnspan=3, sticky=W)
    All_PDF_Button.grid(row=3, columnspan=3, sticky=W)
    All_PowerPoint_Button.grid(row=4, columnspan=3, sticky=W)
    All_Word_Button.grid(row=5, columnspan=3, sticky=W)
    All_TXT_Button.grid(row=6, columnspan=3, sticky=W)
    All_Documet_Type_Button.grid(row=7, columnspan=3, sticky=W)
